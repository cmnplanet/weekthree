...
function question1 () {
  ...
  for (let j = 0; j < price.length; j++){
    ...
    jFix = average.toFixed(2); //two places -- understand the toFixed, round two places in?
  }
  return jFix;
}
...
for(let i=0; i<data.length; i++){
  if(data[i].currency_code === "GBP"){
    nameArray.push(data[i].title);
    priceArray.push(data[i].price);
    }
  }
console.log(nameArray + " costs " + priceArray + " pounds.");
}
...
//   // Answer:
  ...
  for (let i=0; i <data.length; i++){
    ...
    woodContainer.push(data[i].title);
    // for (let j = 0; j < woodContainer.length; j++){
    //   if (woodContainer [i] [j]);
    //     sampling.push(data[i].title);
    //   }
  }
}
console.log(woodContainer);
}
